...
coverage_threshold_3utr = 50
condition = "ISR"
fold_std = 2

## paths:
path_to_density = "./../../downstream_analysis_files_exp_gene_new/density_filtered_genes/"+condition+"/"
if not os.path.exists(path_to_density):
    os.makedirs(path_to_density)

path_to_coverage = "./../../downstream_analysis_files_exp_gene_new/coverage_filtered_genes/"+condition+"/"
if not os.path.exists(path_to_coverage):
    os.makedirs(path_to_coverage)

path_to_frame_info = "./../../downstream_analysis_files_exp_gene_new/periodicity_filtered_genes/"+condition+"/"
if not os.path.exists(path_to_frame_info):
    os.makedirs(path_to_frame_info)

path_to_histogram_data = "./../../downstream_analysis_files_exp_gene_new/length_dist_genes/"+condition+"/"
if not os.path.exists(path_to_histogram_data):
    os.makedirs(path_to_histogram_data)

path_to_final_RT_list = "./../../downstream_analysis_files_exp_gene_new/consolidated_filtered_genes/"+condition+"/"
if not os.path.exists(path_to_final_RT_list):
    os.makedirs(path_to_final_RT_list)

path_to_RT_gene_list = "./../../downstream_analysis_files_exp_gene_new/filtered_genes_frequencies/"+condition+"/"
if not os.path.exists(path_to_RT_gene_list):
    os.makedirs(path_to_RT_gene_list)

path_to_CDS_stats = "./../../downstream_analysis_files_exp_gene_new/cds_stats/"
if not os.path.exists(path_to_CDS_stats):
    os.makedirs(path_to_CDS_stats)

path_to_heatmaps = "./../../downstream_analysis_files_exp_gene_new/heatmaps/"+condition+"/"
...
path_of_RT_graphs = "./../../downstream_analysis_files_exp_gene_new/gene_profiles/"+condition+"/"
...
def function_to_read_BAM_file_again(BAM_filename, transcriptome_annotation, RT_profiles,RT_frame_info,length_range_start,length_range_stop):
	...
	for read in bam_file.fetch(until_eof = True):
		...
		iid = read.reference_name
		if iid not in gene_list or iid not in transcriptome_annotation:
			continue
		pos = read.pos
		length = read.qlen
		#sequence = read.seq
...
with open ("./gene_list.txt") as file:
	...
	for line in file:
		gene=line[:-1]
		if gene not in transcriptome_annotation:
			continue
...
with open (path_to_files_to_run) as file3:
	for line in file3:
		...
		RT_profiles, RT_frame_info = initialise_RT_dictionary(gene_list,transcriptome_annotation)
