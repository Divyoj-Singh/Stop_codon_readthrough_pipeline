...
# global variables
readthrough_abs_reads = 30 # no of reads in isr combining all lengths 
min_reads_in_1_length_of_isr_of_rt_genes=10
ratio_of_den_isr_to_3utr=4

min_no_of_reads_in_cds_for_calculating_cds_data = 200
coverage_threshold_isr = 0.50
coverage_threshold_3utr = 0.25
condition = "ISR"
fold_std = 2

## paths:
path_to_density = "./../../downstream_analysis_files_relaxed/density_filtered_genes/"+condition+"/"
if not os.path.exists(path_to_density):
    os.makedirs(path_to_density)

path_to_coverage = "./../../downstream_analysis_files_relaxed/coverage_filtered_genes/"+condition+"/"
if not os.path.exists(path_to_coverage):
    os.makedirs(path_to_coverage)

path_to_frame_info = "./../../downstream_analysis_files_relaxed/periodicity_filtered_genes/"+condition+"/"
if not os.path.exists(path_to_frame_info):
    os.makedirs(path_to_frame_info)

path_to_histogram_data = "./../../downstream_analysis_files_relaxed/length_dist_genes/"+condition+"/"
if not os.path.exists(path_to_histogram_data):
    os.makedirs(path_to_histogram_data)

path_to_final_RT_list = "./../../downstream_analysis_files_relaxed/consolidated_filtered_genes/"+condition+"/"
if not os.path.exists(path_to_final_RT_list):
    os.makedirs(path_to_final_RT_list)

path_to_RT_gene_list = "./../../downstream_analysis_files_relaxed/filtered_genes_frequencies/"+condition+"/"
if not os.path.exists(path_to_RT_gene_list):
    os.makedirs(path_to_RT_gene_list)

path_to_CDS_stats = "./../../downstream_analysis_files_relaxed/cds_stats/"
if not os.path.exists(path_to_CDS_stats):
    os.makedirs(path_to_CDS_stats)

path_to_heatmaps = "./../../downstream_analysis_files_relaxed/heatmaps/"+condition+"/"
...
path_of_RT_graphs = "./../../downstream_analysis_files_relaxed/gene_profiles/"+condition+"/"
...
path_to_files_to_run = "./../../filenames/files_to_run_downstream/files_to_run_updated_31_1_20.txt"
...
def function_to_get_RT_candidates_first_list(reads_in_frames,iid,transcriptome_annotation,million,length_range_start,length_range_stop):
	...
	for gene in reads_in_frames:
		...
		density_in_cds = (reads_in_cds*1000)/(transcriptome_annotation[gene][5]*million)
		density_in_isr = (reads_in_isr*1000)/(transcriptome_annotation[gene][6]*million)
		density_in_utr3 = (reads_in_utr3*1000)/(transcriptome_annotation[gene][7]*million)
	...
	for i in range(length_range_start,length_range_stop+1):
		for gene in reads_in_frames:
			if sum(reads_in_frames[gene][i]["cds"])>=min_no_of_reads_in_cds_for_calculating_cds_data:
				...
				data_bar_plot[i][0].append(reads_in_frames[gene][i]["cds"][0]/sum(reads_in_frames[gene][i]["cds"]))
				data_bar_plot[i][1].append(reads_in_frames[gene][i]["cds"][1]/sum(reads_in_frames[gene][i]["cds"]))
				data_bar_plot[i][2].append(reads_in_frames[gene][i]["cds"][2]/sum(reads_in_frames[gene][i]["cds"]))
...
with open (path_to_files_to_run) as file3:
	for line in file3:
		...
		good_coverage_genes = [] 
		for gene in coverage:  ### putting threshold on coverage on isr and cds
			if coverage[gene][2] <= coverage_threshold_isr:
				continue
			if coverage[gene][3] >= coverage_threshold_3utr:
				continue
			good_coverage_genes.append(gene)

		function_to_report_readthrough_genes(RT_frame_info,CDS_stats_dict,iid,good_coverage_genes)
		print("Finally done with periodicity analysis!")


potn_RT_list = {}
for filename in os.listdir(path_to_frame_info):
	iid = filename[:-4]
	
	potn_RT_list[iid] = set([])
		
	with open (path_to_final_RT_list+iid+".txt","w") as filew:
		filew.write("gene_id	length	cds_exp_frame0	cds_exp_frame1	cds_exp_frame2	cds_obs_frame0	cds_obs_frame1	cds_obs_frame2	isr_obs_frame0	isr_obs_frame1	isr_obs_frame2	utr3_obs_frame0	utr3_obs_frame1	utr3_obs_frame2	gene_id	length	cds	isr	utr3	# in cds	# in isr	# in utr3	density in cds	density in isr	density in utr3	gene_id	cov(5utr)	cov(cds)	cov(isr)	cov(3utr)\n")
		
		
	with open (path_to_frame_info+iid+".txt") as filef:	
		for linef in filef:
			frame_information = linef[:-1]
			density_information = ""
			coverage_information = ""
			gene_id = frame_information.split("\t")[0]
			potn_RT_list[iid].add(gene_id)
			with open (path_to_density+iid+".txt") as filed:
				for lined in filed:
					if lined.split("\t")[0] == gene_id:
						density_information = lined[:-1]
			with open (path_to_coverage+iid+".txt") as filec:
				for linec in filec:
					if linec.split("\t")[0] == gene_id:
						coverage_information = linec[:-1]
			with open (path_to_final_RT_list+iid+".txt","a+") as filew:
				filew.write(frame_information+"\t"+density_information+"\t"+coverage_information+"\n")

frequency_RT_genes = {}
for i in potn_RT_list:
	for j in potn_RT_list[i]:
		if j not in frequency_RT_genes:
			frequency_RT_genes[j] = 1
		else:
			frequency_RT_genes[j] += 1



with open(path_to_RT_gene_list+condition+".txt","w") as file:

	for i in frequency_RT_genes:
		file.write(i+"\t"+str(frequency_RT_genes[i])+"\n")


transcriptome_annotation = function_to_make_an_annotation_dictionary()
for filename in os.listdir(path_to_frame_info):
	iid = filename[:-4]
	RT_genes_for_graphs = list(potn_RT_list[iid])
	RT_profiles, RT_frame_info = initialise_RT_dictionary(RT_genes_for_graphs,transcriptome_annotation)
	...
	with open (path_to_files_to_run) as file3:
		for line in file3:
			...
			for gene_id_ in RT_profiles:
				## Putting the criterion that first stop codon should be covered.
				if sum(np.array(RT_profiles[gene_id_])[transcriptome_annotation[gene_id_][1]-3:transcriptome_annotation[gene_id_][1]+1]) == 0:
					continue
				## Putting the criterion that second stop codon should be covered.
				if sum(np.array(RT_profiles[gene_id_])[transcriptome_annotation[gene_id_][2]-3:transcriptome_annotation[gene_id_][2]+1]) == 0:
					continue

				plt.plot(np.array(RT_profiles[gene_id_])/million)
				plt.xticks([transcriptome_annotation[gene_id_][0],transcriptome_annotation[gene_id_][1],transcriptome_annotation[gene_id_][2]],['S','*','*'])
				plt.title(gene_id_+"_"+iid+"\n"+str(transcriptome_annotation[gene_id_][0])+"_"+str(transcriptome_annotation[gene_id_][1])+"_"+str(transcriptome_annotation[gene_id_][2]))
				plt.savefig(path_of_RT_graphs+"/whole/"+gene_id_+"_"+iid+".png",dpi = 1000)
				plt.close()
				plt.plot(np.array(RT_profiles[gene_id_][transcriptome_annotation[gene_id_][1]-18:]))
				plt.xticks([15,(transcriptome_annotation[gene_id_][2]-transcriptome_annotation[gene_id_][1]+15)],['*','*'])
				plt.title(gene_id_+"_"+iid+"\n"+str(transcriptome_annotation[gene_id_][0])+"_"+str(transcriptome_annotation[gene_id_][1])+"_"+str(transcriptome_annotation[gene_id_][2]))
				plt.savefig(path_of_RT_graphs+"/zoomed/"+gene_id_+"_"+iid+".png",dpi = 1000)
				plt.close()

				with open(path_of_RT_graphs+"/data/"+gene_id_+"_"+iid+".txt","w") as f:
					for j in np.array(RT_profiles[gene_id_]):
						f.write(str(j)+"\n")
